fetch("./Productos.csv")
    ...
    .then(data => {
        ...
        for (let i = 1; i < lineas.length; i++) {
            ...
            if (categoria !== "Manillas") continue;

            catalogo.innerHTML += `
                <div class="producto">
                    <img src="Productos/Manillas/${foto}" alt="Manilla ${contador}">
                    <h3>Manilla ${contador}</h3>
                    <p class="precio">$${precio}</p>

                    <button onclick="comprar('Manilla ${contador}','${precio}')">
                        Lo Quiero
                    </button>
                </div>
            `;
